...
static void
luaK_patchlistaux (FuncState * fs, int list, int ttarget, int treg,
                   int ftarget, int freg, int dtarget)
{
  while (list != NO_JUMP)
    {
...
addk (FuncState * fs, TObject * k, TObject * v)
{
  ...
  if (ttisnumber (idx))
    ...
    {                           /* constant not found; create a new entry */
      Proto *f = fs->f;
      luaM_growvector (fs->L, f->k, fs->nk, f->sizek, TObject, MAXARG_Bx,
  ...
  if (op <= OPR_POW)
    ...
    {                           /* test operator */
      static const OpCode ops[] =
        { OP_EQ, OP_EQ, OP_LT, OP_LE, OP_LT, OP_LE };
      int cond = 1;
      if (op >= OPR_GT)
        {                       /* `>' or `>='? */
