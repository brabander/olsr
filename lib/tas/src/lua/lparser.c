...
{
  luaX_syntaxerror (ls,
                    luaO_pushfstring (ls->L, "`%s' expected",
...
*/
static BinOpr
subexpr (LexState * ls, expdesc * v, int limit)
{
  BinOpr op;
  UnOpr uop;
  enterlevel (ls);
...
{
  expdesc e;
  check_condition (ls, VLOCAL <= lh->v.k
                   && lh->v.k <= VINDEXED, "syntax error");
  if (testnext (ls, ','))
...
forbody (LexState * ls, int base, int line, int nvars, int isnum)
{
  ...
  luaK_patchtohere (fs, prep - 1);
    ...
    (isnum) ? luaK_codeAsBx (fs, OP_FORLOOP, base,
                             NO_JUMP) : luaK_codeABC (fs, OP_TFORLOOP, base,
                                                      0, nvars - 3);
...
fornum (LexState * ls, TString * varname, int line)
{
  ...
  luaK_codeABC (fs, OP_SUB, fs->freereg - 3, fs->freereg - 3,
                fs->freereg - 1);
  luaK_jump (fs);
