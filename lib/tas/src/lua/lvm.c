...
static void
callTMres (lua_State * L, const TObject * f, const TObject * p1,
           const TObject * p2)
{
...
static void
callTM (lua_State * L, const TObject * f, const TObject * p1,
        const TObject * p2, const TObject * p3)
{
  setobj2s (L->top, f);         /* push function */
  setobj2s (L->top + 1, p1);    /* 1st argument */
  setobj2s (L->top + 2, p2);    /* 2nd argument */
...
static int
call_binTM (lua_State * L, const TObject * p1, const TObject * p2, StkId res,
            TMS event)
{
...
static int
call_orderTM (lua_State * L, const TObject * p1, const TObject * p2,
              TMS event)
{
    ...
    case LUA_TUSERDATA:
      {
        if (uvalue (t1) == uvalue (t2))
          return 1;
        tm =
          get_compTM (L, uvalue (t1)->uv.metatable, uvalue (t2)->uv.metatable,
                      TM_EQ);
        break;                  /* will try TM */
      }
    case LUA_TTABLE:
      {
        if (hvalue (t1) == hvalue (t2))
          return 1;
        tm =
          get_compTM (L, hvalue (t1)->metatable, hvalue (t2)->metatable,
                      TM_EQ);
        break;                  /* will try TM */
      }
    default:
  ...
  do
    {
      ...
      if (!tostring (L, top - 2) || !tostring (L, top - 1))
        ...
        {                       /* if len=0, do nothing */
          ...
          lu_mem tl =
            cast (lu_mem, tsvalue (top - 1)->tsv.len) + cast (lu_mem,
                                                              tsvalue (top -
                                                                       2)->
                                                              tsv.len);
          char *buffer;
          int i;
          while (n < total && tostring (L, top - n - 1))
...
static void
Arith (lua_State * L, StkId ra, const TObject * rb, const TObject * rc,
       TMS op)
{
  TObject tempb, tempc;
  const TObject *b, *c;
      ...
      && (c = luaV_tonumber (rc, &tempc)) != NULL)
    {
      switch (op)
        ...
        case TM_POW:
          {
            const TObject *f =
              luaH_getstr (hvalue (gt (L)), G (L)->tmname[TM_POW]);
        ...
        case OP_TFORLOOP:
          {
            ...
            cb = ra + nvar;
            do
              {                 /* move results to proper positions */
                nvar--;
