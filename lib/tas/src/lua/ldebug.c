...
static int
auxgetinfo (lua_State * L, const char *what, lua_Debug * ar, StkId f,
            CallInfo * ci)
{
  int status = 1;
  for (; *what; what++)
...
{
  return (r < pt->maxstacksize
          || (r >= MAXSTACK && r - MAXSTACK < pt->sizek));
}
...
luaG_typeerror (lua_State * L, const TObject * o, const char *op)
{
  ...
  const char *kind =
    (isinstack (L->ci, o)) ? getobjname (L->ci, o - L->base, &name) : NULL;
  if (kind)
