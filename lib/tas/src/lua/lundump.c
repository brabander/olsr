...
static void
ezread (LoadState * S, void *b, int n)
{
...
static void
LoadVector (LoadState * S, void *b, int m, size_t size)
{
  if (S->swap)
    {
      char *q = (char *) b;
...
static void
LoadLocals (LoadState * S, Proto * f)
{
  int i, n;
  n = LoadInt (S);
...
LoadUpvalues (LoadState * S, Proto * f)
{
  ...
  n = LoadInt (S);
  if (n != 0 && n != f->nups)
    luaG_runerror (S->L, "bad nupvalues in %s: read %d; expected %d", S->name,
                   n, f->nups);
...
static void
TestSize (LoadState * S, int s, const char *what)
{
  int r = LoadByte (S);
  if (r != s)
    luaG_runerror (S->L,
...
LoadHeader (LoadState * S)
{
  ...
  if (version > VERSION)
    luaG_runerror (S->L,
                   "%s too new: "
                   "read version %d.%d; expected at most %d.%d", S->name,
                   V (version), V (VERSION));
  if (version < VERSION0)       /* check last major change */
    luaG_runerror (S->L,
                   "%s too old: "
                   "read version %d.%d; expected at least %d.%d", S->name,
                   V (version), V (VERSION0));
...
*/
Proto *
luaU_undump (lua_State * L, ZIO * Z, Mbuffer * buff)
{
  LoadState S;
