...
int
luaD_pcall (lua_State * L, Pfunc func, void *u, ptrdiff_t old_top,
            ptrdiff_t ef)
{
  int status;
...
f_parser (lua_State * L, void *ud)
{
  ...
  tf =
    p->bin ? luaU_undump (L, p->z, &p->buff) : luaY_parser (L, p->z,
                                                            &p->buff);
