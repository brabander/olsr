...
init_zebra (void)
{
  ...
  if (!(zebra.status & STATUS_CONNECTED))
    olsr_exit
      ("(QUAGGA) AIIIII, could not connect to zebra! is zebra running?",
       EXIT_FAILURE);
}
...
zebra_route_packet (struct ipv4_route r, ssize_t * optlen)
{
  ...
  *t++ = r.message;
  *t++ = r.prefixlen;
  for (count = 0; count < r.prefixlen / 8 + (r.prefixlen % 8 ? 1 : 0);
       count++)
    {
  ...
  do
    {
      ...
      if (!ret)
        {                       // nothing more to read, packet is broken, discard!
          free (buf);
          return NULL;
        }
...
zebra_parse_packet (unsigned char *packet, ssize_t maxlen)
{
  ...
  parse_interface_add, parse_interface_delete, parse_interface_address_add,
      ...
      parse_interface_down, ipv4_route_add, ipv4_route_delete,
...
static int
parse_interface_add (unsigned char *opt __attribute__ ((unused)), size_t len
...
static int
parse_interface_delete (unsigned char *opt
...
static int
parse_interface_address_add (unsigned char *opt
                             __attribute__ ((unused)), size_t len
                             __attribute__ ((unused)))
{
...
static int
parse_interface_up (unsigned char *opt __attribute__ ((unused)), size_t len
                    __attribute__ ((unused)))
{
...
static int
parse_interface_down (unsigned char *opt __attribute__ ((unused)), size_t len
                      __attribute__ ((unused)))
{
...
static int
parse_interface_address_delete (unsigned char *opt
                                __attribute__ ((unused)), size_t len
                                __attribute__ ((unused)))
{
  //todo
  ...
  if (r->message & ZAPI_MESSAGE_NEXTHOP)
    {
      ...
      if (len < (sizeof (uint32_t) + 1) * r->nh_count)
        return -1;
      r->nexthops =
        olsr_malloc ((sizeof r->nexthops->type +
  ...
  if (r->message & ZAPI_MESSAGE_IFINDEX)
    {
      ...
      if (len < sizeof (uint32_t) * r->ind_num)
        return -1;
      r->index =
        olsr_malloc (sizeof (uint32_t) * r->ind_num,
                     "quagga: parse_ipv4_route_add");
...
static int
parse_ipv6_route_add (unsigned char *opt __attribute__ ((unused)), size_t len
                      __attribute__ ((unused)))
{
  //todo
  return 0;
...
zebra_add_olsr_v4_route (const struct rt_entry *r)
{
  ...
  if ((r->rt_best->rtp_nexthop.gateway.v4.s_addr == r->rt_dst.prefix.v4.s_addr
       && route.prefixlen == 32))
    {
      ...
      route.ind_num = 1;
      route.index =
        olsr_malloc (sizeof *route.index, "zebra_add_olsr_v4_route");
      *route.index = htonl (r->rt_best->rtp_nexthop.iif_index);
      route.nexthops =
        olsr_malloc (sizeof route.nexthops->type +
