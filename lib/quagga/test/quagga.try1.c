...
static void update_olsr_zebra_routes (struct ipv4_route *,
                                      struct ipv4_route *);
  ...
  if (r->message & ZAPI_MESSAGE_NEXTHOP)
    {
      ...
      if (len < sizeof (uint32_t) * r->nh_count)
        return -1;
      r->nexthops =
        olsr_malloc (sizeof (uint32_t) * r->nh_count,
          ...
          for (; n != &e->networks; n = n->next)
            {
              if (!r)
                {
                  r =
                    zebra_create_ipv4_route_table_entry (n->A_network_addr.v4,
                                                         n->A_netmask.v4,
                                                         e->A_gateway_addr.
                                                         v4);
                  t = r;
...
static struct ipv4_route *
zebra_create_ipv4_route_table_entry (uint32_t addr, uint32_t mask,
                                     uint32_t gw)
{
  ...
  r->nh_count = 1;
  r->nexthops =
    olsr_malloc (sizeof (uint32_t), "zebra_create_ipv4_route_table_entry");
  *r->nexthops = gw;
  r->next = NULL;
...
while (mask & (1 << ++prefixlen && prefixlen < 32); return prefixlen;}
       ...
       for (t = r; t; t = t->next)
       {
       if (a->prefix == t->prefix) if (a->prefixlen ==
                                       t->prefixlen) if (*a->nexthops ==
                                                         *t->nexthops)
       {
       goto foo;}
       }
  ...
  foo:
       ...
       for (t = a; t; t = t->next)
       {
       if (r->prefix == t->prefix) if (r->prefixlen ==
                                       t->prefixlen) if (*r->nexthops ==
                                                         *t->nexthops)
       {
       goto bar;}
       }
